...
describe("ProviderRuntimeIngestion", () => {
  ...
  it("applies an authoritative provider title over a generated title", async () => {
    const harness = await createHarness({ threadTitle: DEFAULT_THREAD_TITLE });
    await harness.dispatch({
      type: "thread.title.generate.complete",
      commandId: CommandId.make("cmd-initial-generated-title"),
      threadId: asThreadId("thread-1"),
  ...
  it("preserves a manual title when Gemini updates its topic", async () => {
    const harness = await createHarness();
    await harness.dispatch({
    ...
    await harness.emitAndDrain([
      {
        type: "thread.metadata.updated",
        eventId: asEventId("evt-thread-metadata-manual"),
        provider: ProviderDriverKind.make("gemini"),
        createdAt: "2026-01-01T00:00:00.000Z",
        threadId: asThreadId("thread-1"),
        payload: {
