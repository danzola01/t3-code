 ...
 * `~/.grok/sessions/**\/updates.jsonl`, and Gemini CLI chat session files)
 * rather than relying on T3 Code's own orchestration projections, so usage stays
 ...
 *
 * v5 adds `grok` and v6 adds `gemini` to {@link UsageProviderKind}; v4
...
export const UsageProviderKind = Schema.Literals(["claude", "codex", "grok", "gemini"]);
