...
Opening a branch picker refreshes branches and the current checkout from Git on the server. Reopen
the picker after creating, deleting, fetching, or checking out a branch in a terminal. Mobile also
refreshes when the branch screen opens and supports pull-to-refresh. Existing threads remain tied
to their branch; a different checkout in a shared workspace is shown as a mismatch.
...
Collapse the **Others** section to keep authored and review-requested work in focus.
