...
Gemini is a fork-specific vertical slice under `apps/server/src/provider/gemini/`. Its driver,
ACP spawn and authentication policy, adapter, catalog bridge, snapshot probing, and text generation
stay together; the shared registry has one additive driver import. Gemini instances use the open
`providerInstances` settings envelope instead of widening the legacy closed provider map.
...
Gemini's ACP implementation can replace tool-call content while retaining arguments only in the
initial update. Its runtime opts into preserving that initial content so MCP activity remains
structured. Gemini `update_topic` tool calls are normalized into authoritative thread-title updates.
